...
int main()
{
	...
	ReadClientConfigFile("../ini/CVFTXODB.ini", strListenPort, strHeartBeatTime, strEPIDNum);

	int nService = 0;
	printf("Quote program start\n");
	ReadConfigFile("../ini/CVFTXODB.ini", "EXCHANGE", struTSConfig);
	...
	//Queue init.
	ReadQueueDAOConfigFile("../ini/CVFTXODB.ini", strService, nNumberOfQueueDAO, kQueueDAOWriteStartKey, kQueueDAOWriteEndKey, kQueueDAOReadStartKey, kQueueDAOReadEndKey, kQueueDAOMonitorKey);
