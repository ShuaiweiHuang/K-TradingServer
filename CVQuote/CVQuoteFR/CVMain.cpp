...
int main()
{
	...
	ReadClientConfigFile("../ini/CVQuoteFR.ini", strListenPort, strHeartBeatTime, strEPIDNum);

	int nService = 0;
	printf("Quote program start\n");
	ReadConfigFile("../ini/CVQuoteFR.ini", "EXCHANGE", struTSConfig);
	...
	//Queue init.
	ReadQueueDAOConfigFile("../ini/CVQuoteFR.ini", strService, nNumberOfQueueDAO, kQueueDAOWriteStartKey, kQueueDAOWriteEndKey, kQueueDAOReadStartKey, kQueueDAOReadEndKey, kQueueDAOMonitorKey);
