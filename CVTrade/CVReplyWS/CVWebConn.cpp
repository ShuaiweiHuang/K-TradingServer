	...
	if(str[0] == '{') {
		...
		if(jtable["table"] != "margin" && jtable["table"] != "position" &&  jtable["cv_exchange"].dump() != "null") {
			...
			if(	jtable["cv_exchange"] == "BINANCE" ||
				jtable["cv_exchange"] == "BITMEX" ||
				jtable["cv_exchange"] == "FTX" ||
				...
				FprintfStderrLog("[UnknownExchange]", -1, 0, jtable["cv_exchange"].dump().c_str(), jtable["cv_exchange"].dump().length(),  NULL, 0);
			}
		}
