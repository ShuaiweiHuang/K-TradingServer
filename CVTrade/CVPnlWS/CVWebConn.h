...
struct CV_StructTSPNLReply
{
	char status_code[4];
	char accounting_no[7];
	char strategy[30];
	char position[10];
	char symbol[10];
	char exchange[10];
...
class CCVServer: public CCVThread, public ICVClientSocketCallback, public ICVHeartbeatCallback, public ICVRequestCallback
{
	private:
		...
		static void OnData_Order_Pnl(client* c, websocketpp::connection_hdl, client::message_ptr msg);
		void Position_Update(json*);
	...
	protected:
		...
		struct CV_StructTSPNLReply m_pnl_reply;
