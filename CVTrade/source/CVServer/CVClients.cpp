...
CCVClients::CCVClients() 
{
	...
	ifstream NodeFile("PROXY_NODE");

	assert(NodeFile);

#ifdef MNTRMSG
	char text[12], filename[30];
	time_t now = time(NULL);
	struct tm *t = localtime(&now);
	strftime(text, sizeof(text)-1, "%Y%m%d%H", t);

	sprintf(filename, "../../log/Order_%s.csv", text);
	m_fileOrder.open(filename, ios::out);
	m_fileOrder << "Length,User ID,Account,Unit No.,Ctl ID,Seq No.,Key,Agent,User IP,Keyin ID,Func Code,Step Mark,Buy/Sell,Company ID,Quantity,Price,Order Type,Price Type,Time in Force,Sale No,Trade Kind" << endl;

	sprintf(filename, "../../log/Reply_%s.csv", text);
	m_fileReply.open(filename, ios::out);
	m_fileReply << "Length,Account,Key,Order No,Status,Reply Message" << endl;
#endif
