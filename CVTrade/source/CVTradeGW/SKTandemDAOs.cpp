...
CSKTandemDAO* CSKTandemDAOs::GetAvailableDAO()
{
	...
	{
		if(m_vTandemDAO[m_nRoundRobinIndexOfTandemDAO]->GetStatus() == tsServiceOn && m_vTandemDAO[m_nRoundRobinIndexOfTandemDAO]->IsInuse() == false)
		{
...
void CSKTandemDAOs::RemoveNotAvailableDAO()//not finished
{
	for(vector<CSKTandemDAO*>::iterator iter = m_vTandemDAO.begin(); iter != m_vTandemDAO.end(); iter++)
	{
		if((*iter)->GetStatus() == tsBreakdown)
		{
			delete *iter;
			*iter = NULL;

			m_vTandemDAO.erase(iter);
		}
	}
}
...
void CSKTandemDAOs::SetConfiguration(string strService, int nInitialConnection, int nMaximumConnection,
