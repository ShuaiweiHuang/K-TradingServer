...
void* test_run(void *arg)
{
	...
	for(exec_loop=0 ; (exec_loop < 1) && is_conn ; exec_loop++)
	{
		...
		if(is_login && is_conn) {
			...
			memcpy(ts_order.touch_price, "097000000", 9);
			memcpy(ts_order.qty_mark, "0", 1);
			memcpy(ts_order.order_qty, "000000500", 9);
			memcpy(ts_order.order_kind,"0", 1);

			memcpy(ts_order.order_buysell_oco, "S", 1);
			memcpy(ts_order.order_cond_oco, "0", 1);//0:ROD
			memcpy(ts_order.order_mark_oco, "1", 1);//0:Market 1:limit 2:protect 3:stop market 4:stop limit
			memcpy(ts_order.trade_type_oco, "0", 1);//0:new 1:delete 2:delete all 3:change qty 4:change price
			memcpy(ts_order.order_bookno_oco, "000000000000000000000000000000000000", 36);
			memcpy(ts_order.price_mark_oco, "0", 1);
			memcpy(ts_order.order_price_oco, "099700000", 9);
			memcpy(ts_order.touch_price_oco, "096000000", 9);
			memcpy(ts_order.qty_mark_oco, "0", 1);
			memcpy(ts_order.order_qty_oco, "000000000", 9);
			...
			//memset(&ts_order.reserved, ' ', 91);
				...
				memcpy(ts_order.order_bookno_oco, data1+185, 36);
				printf("%.36s\n", ts_order.order_bookno_oco);
