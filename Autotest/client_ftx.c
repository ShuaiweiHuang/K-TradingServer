	...
	for(exec_loop=0 ; (exec_loop < 1) && is_conn ; exec_loop++)
	{
		...
		if(is_login && is_conn) {
			...
			for(order_loop=0 ; order_loop<order_num && is_conn ; order_loop++)
			{
				...
				memcpy(ts_order.strategy_name, "KEANU_BTCPERP_FTX\0", sizeof(ts_order.strategy_name));
				memcpy(ts_order.agent_id, "MC", 2);
				memcpy(ts_order.broker_id, "9801", 4);
				memcpy(ts_order.exchange_id, "FTX\0", 10);
				memcpy(ts_order.seq_id, "1234567890123", 13);
				memcpy(ts_order.symbol_name, "BTC-PERP\0", 10);
				...
				memcpy(ts_order.order_date, "20200226", 8);
				...
				memcpy(ts_order.order_price, "090600000", 9);
				memcpy(ts_order.touch_price, "000000000", 9);
