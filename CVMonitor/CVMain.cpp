...
int main()
{
	...
	ReadClientConfigFile("../ini/CVMonitor.ini", strListenPort, strHeartBeatTime, strEPIDNum);

	int nService = 0;
	printf("Monitor program start\n");
#if 0
	ReadConfigFile("../ini/CVMonitor.ini", "EXCHANGE", struTSConfig);
...
#endif	
	//Queue init.
	ReadQueueDAOConfigFile("../ini/CVMonitor.ini", strService, nNumberOfQueueDAO, kQueueDAOWriteStartKey, kQueueDAOWriteEndKey, kQueueDAOReadStartKey, kQueueDAOReadEndKey);
