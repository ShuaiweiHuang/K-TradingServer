...
void* test_run(void *arg)
{
	...
	SSL_CTX *ctx = NULL;
	SSL		*ssl = NULL;
	int		server = -1, ret = 0, is_conn = 1, usermode = 0;
...
#endif
		    int group_loop;
			for(group_loop = 0 ; group_loop<1 ; group_loop++)
			{

				for(order_loop=0 ; order_loop<order_num && is_conn ; order_loop++)
				{
					sprintf(&ts_order.reserved, "testing in order %d", order_loop);
					if(usermode)
						sleep(rand()%WAITUSERMODE+1);

					int ntowrite = 320, nwrite = 0, len;

					while(ntowrite) {
						if ((len = SSL_write(ssl, (&ts_order.prefix[0+nwrite]), ntowrite)) <= 0) {
							perror("write to server error !");
							is_conn = 0;
							ret = -1;
							break;
				...
				int nread = 0, ntoread = 400*order_num;
				...
				while(ntoread){
					...
					ntoread -= len;
					nread += len;
					printf("order data num=%d, %x, %x, %.318s,\n%.78s\nstatus:%.c\n", nread/400, data1[0], data1[1], data1+2, data1+320, data1+398, data1+399);
				}
				printf("readnum = %d\n", nread/400);
			}
#if TIMEDIFF
