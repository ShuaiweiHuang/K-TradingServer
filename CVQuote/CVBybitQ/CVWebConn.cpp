...
void CCVServer::OnData_BYBIT(client* c, websocketpp::connection_hdl con, client::message_ptr msg)
{
	...
	{ 
		...
		time_str   = jtable["data"][i]["timestamp"].dump();
		...
		sprintf(netmsg, "01_ID=%s.BYBIT,ECC.1=%d,Time=%s,C=%s,V=%s,TC=%d,EPID=%s,ECC.2=%d,EPOCH=%s,",
...
void CCVServer::OnHeartbeatRequest()
{
	...
	if(m_heartbeat_count <= HTBT_COUNT_LIMIT)
	{
		if(m_strName == "BYBIT")
		{
			if(!m_BYBIT_enable)
			{
				auto j = json::parse("{ \"op\": \"subscribe\", \"args\": [\"trade.BTCUSD|ETHUSD\"]}");
